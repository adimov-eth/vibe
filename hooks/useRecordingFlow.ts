...
const getUsageHelper = () => {
  return {
    async checkCanCreateConversation(): Promise<boolean> {
      ...
      if (!usageStats) {
        console.error('[RecordingSession] No usage stats available to check conversation limit');
        return false;
      }
...
class RecordingSession {
  ...
  private constructor(modeId: string) {
    ...
    this._modeId = modeId;
    console.log(`[RecordingSession] Created new session with localId=${this._localId}, modeId=${modeId}`);
    this.setupAudio();
  }
  
  public static getInstance(modeId: string): RecordingSession {
    if (!RecordingSession.instance || RecordingSession.instance._modeId !== modeId) {
  ...
  private setFlowState(state: RecordingFlowState): void {
    if (this._flowState !== state) {
      console.log(`[RecordingSession] State change: ${this._flowState} -> ${state}`);
      this._flowState = state;
      // Notify all subscribers
      this._stateChangeCallbacks.forEach(callback => callback(state));
    }
  }
  
  private setError(error: string | null): void {
    if (this._error !== error) {
      console.log(`[RecordingSession] Error change: ${this._error} -> ${error}`);
      this._error = error;
      // Notify all subscribers
      this._errorChangeCallbacks.forEach(callback => callback(error));
    }
  }
  
  public onStateChange(callback: (state: RecordingFlowState) => void): () => void {
    this._stateChangeCallbacks.push(callback);
    return () => {
      this._stateChangeCallbacks = this._stateChangeCallbacks.filter(cb => cb !== callback);
    };
  }
  ...
  public onErrorChange(callback: (error: string | null) => void): () => void {
    ...
    return () => {
      this._errorChangeCallbacks = this._errorChangeCallbacks.filter(cb => cb !== callback);
    };
  }
  ...
  public async toggleRecording(): Promise<void> {
    if (this._operationInProgress) {
      console.warn("[RecordingSession] Operation already in progress. Skipping toggleRecording request.");
      return;
    }
    
    if (!this._isAudioSetupComplete) {
      console.warn("[RecordingSession] toggleRecording called before audio setup complete. Ignoring.");
    ...
    this.setError(null);
    console.log(`[RecordingSession] toggleRecording called. Current state: ${this._flowState}`);
    ...
    try {
      ...
      if (this._flowState === 'recording') {
        ...
        console.log(`[RecordingSession] Recording stopped successfully. URI: ${uri}`);
        
        this.setFlowState('savingIntent');
        const audioKey = this._recordMode === 'live' ? 'live' : this._currentPartner.toString();
        
        const store = useStore.getState();
        await store.saveUploadIntent(this._localId, uri, audioKey);
        console.log(`[RecordingSession] Saved upload intent for localId ${this._localId}, audioKey ${audioKey}`);
        
        const isLastRecordingStep = (this._recordMode === 'live') || (this._recordMode === 'separate' && this._currentPartner === 2);
        
        if (isLastRecordingStep) {
          console.log('[RecordingSession] Last recording step finished.');
          const currentServerId = useStore.getState().localToServerIds[this._localId];
          console.log(`[RecordingSession] Setting state to ${currentServerId ? 'complete' : 'waitingForServerId'}`);
          this.setFlowState(currentServerId ? 'complete' : 'waitingForServerId');
        } else {
          this._currentPartner = 2;
          console.log('[RecordingSession] Separate mode: Switched to Partner 2. Setting state to readyToRecord.');
          this.setFlowState('readyToRecord');
        }
      }
      // Start recording flow
      else if (this._flowState === 'idle' || this._flowState === 'readyToRecord') {
        ...
        if (!canCreate) {
          console.error('[RecordingSession] START: Cannot create conversation (usage limit).');
        ...
        if (!hasPermission) {
          console.error('[RecordingSession] START: Microphone permission denied.');
        ...
        const existingServerId = useStore.getState().localToServerIds[this._localId];
        
        if (isFirstRecordingAttempt && !existingServerId) {
          console.log(`[RecordingSession] START: First recording attempt and no server ID. Creating conversation: localId=${this._localId}`);
          console.log('[RecordingSession] START: Setting state to creatingConversation');
          this.setFlowState('creatingConversation');
          
          // Create conversation in background
          const store = useStore.getState();
          const creationPromise = store.createConversation(this._modeId, this._recordMode, this._localId);
          
          // Set ready state before waiting for conversation creation
          console.log('[RecordingSession] START: Setting state to readyToRecord');
          this.setFlowState('readyToRecord');
          
          // Handle conversation creation result
          creationPromise.then(() => {
            console.log(`[RecordingSession] START: Conversation creation completed for localId: ${this._localId}.`);
          }).catch(createErr => {
            console.error(`[RecordingSession] START: Conversation creation error: ${createErr}. Recording will continue anyway.`);
          });
        } else {
          console.log(`[RecordingSession] START: Not first recording attempt or server ID (${existingServerId}) exists. Skipping createConversation.`);
          console.log('[RecordingSession] START: Setting state to readyToRecord');
          this.setFlowState('readyToRecord');
        }
        
        // Start recording
        console.log('[RecordingSession] START: Calling recordingService.startRecording()...');
        try {
          await recordingService.startRecording();
          console.log('[RecordingSession] START: recordingService.startRecording() completed successfully.');
          console.log('[RecordingSession] START: Setting state to recording');
          this.setFlowState('recording');
        } catch (startErr) {
          console.error('[RecordingSession] START: startRecording failed:', startErr);
        ...
        console.warn(`[RecordingSession] toggleRecording called in unexpected state: ${this._flowState}`);
      }
    } catch (err) {
      console.error('[RecordingSession] Error during recording operation:', err);
      const message = err instanceof Error ? err.message : String(err);
      this.setError(`Recording operation failed: ${message}`);
      this.setFlowState('error');
      await recordingService.cleanupCurrentRecording(this._lastRecordingUri || undefined);
    } finally {
  ...
  public async cleanup(): Promise<void> {
    ...
    if (this._flowState === 'recording') {
      try {
        ...
        console.error("[RecordingSession] cleanup: Error stopping recording:", err);
      }
    }
  ...
  public checkForUpdatedServerId(): void {
    if (this._flowState === 'waitingForServerId') {
      ...
      if (serverId) {
        console.log(`[RecordingSession] Server ID ${serverId} detected while waiting. Transitioning to complete.`);
...
export const useRecordingFlow = ({ modeId }: UseRecordingFlowProps): RecordingFlowResult => {
  ...
  const [flowState, setFlowState] = useState<RecordingFlowState>(session.flowState);
  ...
  const isButtonDisabled = ![
    ...
    'complete',
    'error'
  ].includes(flowState) || !session.isAudioSetupComplete || session.operationInProgress;
  
  // Mode Toggle Handler
  const handleToggleMode = useCallback((index: number) => {
    if (flowState !== 'idle' && flowState !== 'readyToRecord' && flowState !== 'error') return;
