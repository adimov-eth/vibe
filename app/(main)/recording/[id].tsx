...
function RecordingScreen() { // Renamed internally for clarity with memo
  ...
  useEffect(() => {
    ...
    if (flowState === 'complete' && serverId) {
      console.log(`[RecordingScreen] Flow complete and serverId ${serverId} known. Navigating to results...`);
      router.replace(`../results/${serverId}`);
    } else if (flowState === 'waitingForServerId') {
        console.log(`[RecordingScreen] Flow state is 'waitingForServerId'. Waiting for serverId mapping...`);
  ...
  useEffect(() => {
    return () => {
      console.log("[RecordingScreen] Component unmounting. Hook cleanup should handle AV resources.");
  ...
  const getProcessingText = (): string => {
       switch(flowState) {
           ...
           case 'creatingConversation': return 'Preparing conversation...';
           case 'stopping': return 'Stopping recording...';
           case 'savingIntent': return 'Saving recording...';
           case 'waitingForServerId': return 'Finalizing...';
  ...
  const isToggleDisabled = ![
       'idle',
       'readyToRecord',
       'error', // Allow toggling even on error to potentially switch modes
       'complete' // Allow toggle after completion? Maybe not. Let's disable.
   ].includes(flowState);
  ...
  return (
    <Container withSafeArea>
      ...
      <View style={styles.content}>
        ...
        {recordMode === 'separate' && !showProcessingIndicator && flowState !== 'recording' && ( // Show only when idle/ready/error in separate mode
          ...
          {showProcessingIndicator ? (
            <View style={styles.processingContainer}>
              ...
              <View style={styles.errorContainer}>
                 ...
                 <Text style={styles.errorText}>{recordingError || 'An unknown error occurred.'}</Text>
            ...
            <>
              ...
              <Text style={styles.recordingInstructions}>
                 {flowState === 'recording' ? 'Recording... Tap to stop' :
                  flowState === 'readyToRecord' || flowState === 'idle' ? 'Tap to start recording' :
                  ''}
              </Text>
            </>
          )}
        ...
        {!showProcessingIndicator && flowState !== 'error' && ( // Hide waveform during processing and error
...
const styles = StyleSheet.create({
  ...
  waveformContainer: { height: 120, marginVertical: spacing.lg, justifyContent: 'center', alignItems: 'center' }, // Added center alignment
  processingContainer: { alignItems: 'center', justifyContent: 'center', flex: 1 }, // Ensure it takes space
