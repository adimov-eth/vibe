...
if (!API_URL) {
  console.error('[API Client] EXPO_PUBLIC_API_URL environment variable is not set.');
...
export const fetchWithAuth = async <T = any>(
  ...
  if (!authHeader) {
    console.error(`[API Client] No auth header available for ${url}. Throwing AuthenticationError.`);
  ...
  try {
    ...
    if (response.status === 401) {
      console.warn(`[API Client] Received 401 Unauthorized for ${url}. Throwing AuthenticationError.`);
    ...
    if (!response.ok) {
      const errorMessage = `API Error: ${response.status} ${response.statusText || ''}`.trim();
      console.error(`[API Client] Request failed for ${url}: ${errorMessage}`, responseBody);
      throw new ApiError(errorMessage, response.status, responseBody);
    }

    console.log(`[API Client] Request successful: ${options.method || 'GET'} ${url} (Status: ${response.status})`);
