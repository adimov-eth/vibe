...
export const uploadAudioInBackground = async (
  ...
  localToServerIds: LocalToServerIdMap
    ...
    console.log(`[BackgroundUtil:uploadAudioInBackground] STARTING background upload for ServerConvID=${serverConversationId}, LocalConvID=${localConversationId || 'N/A'}, Key=${audioKey}, Attempt=${pendingUpload.attemptCount + 1}`);
    ...
    try {
        ...
        if (!authHeader) {
            console.error('[BackgroundUtil:uploadAudioInBackground] Failed to get authorization header. Cannot proceed. Will retry later.');
    ...
    try {
      ...
      if (response?.status === 401) {
          console.error(`[BackgroundUtil:uploadAudioInBackground] Received 401 Unauthorized for ${serverConversationId}_${audioKey}. Auth token likely expired. Will retry later.`);
  ...
  try {
    ...
    for (const upload of uploadsToProcess) {
        ...
        if (upload.attemptCount >= MAX_BACKGROUND_ATTEMPTS) {
            console.warn(`[BackgroundTask:${BACKGROUND_UPLOAD_TASK}] Skipping upload ${upload.audioKey} for ${upload.conversationId} - Max attempts (${MAX_BACKGROUND_ATTEMPTS}) reached.`);
            skippedCount++;
            continue;
        }
    ...
    console.log(`[BackgroundTask:${BACKGROUND_UPLOAD_TASK}] TASK FINISHED in ${taskEndTime - taskStartTime}ms. Processed: ${processedCount}, Success: ${successCount}, Failed: ${failureCount}, Skipped: ${skippedCount}. Remaining Pending: ${finalPending.length}`);
